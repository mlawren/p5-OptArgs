...
Used to put an opt into another package than the current one.

=item ishelp

When true flags this option as a help option, which when given will
then result in an internal "die usage()" call. This is basically a
...
Used to put an arg into another package than the current one.
