...
in contrast with modules in the Getopt::* namespace, which deal with
options only.  The following model is assumed by B<OptArgs> for
...
indistinguishable from arguments and options to the main Command.
Sub-command options and arguments are defined in the same place as
...
based on the value of a particular argument. To use sub-commands
you build your application with the following structure:
...
than dispatch to your Command Class, and eventually a Sub-Command
Class.
...
When true this option will be accepted as normal but will not appear in
any usage messages. This is handy if you have developer-only options,
or options that are very rarely used that you don't want cluttering up
your usage message.
...
Parse C<@ARGV> (or C<@argv> if given) and dispatch to C<$function>
in the appropriate package name constructed from C<$rootclass>.
