...
sub OptArgs2::Cmd_CI::import {shift;my$ref={@_ > 1 ? @_ : %{$_[0]}};$OptArgs2::Cmd::_CI=exists$ref->{'has'}? $ref : {has=>$ref }}package OptArgs2::Cmd;BEGIN {require OptArgs2::CmdBase;our@ISA=('OptArgs2::CmdBase')};require Carp;our%_ATTRS;my%_BUILD_CHECK;sub new {my$class=shift;my$self={@_ ? @_ > 1 ? @_ : %{$_[0]}: ()};%_ATTRS=map {($_=>1)}keys %$self;bless$self,ref$class || $class;$_BUILD_CHECK{$class}//= do {my@possible=($class);my@BUILD;my@CHECK;while (@possible){no strict 'refs';my$c=shift@possible;push@BUILD,$c .'::BUILD' if exists &{$c .'::BUILD'};push@CHECK,$c .'::__CHECK' if exists &{$c .'::__CHECK'};push@possible,@{$c .'::ISA'}}[reverse(@CHECK),reverse(@BUILD)]};map {$self->$_}@{$_BUILD_CHECK{$class}};Carp::carp("OptArgs2::Cmd attribute '$_' unexpected")for keys%_ATTRS;$self}sub __RO {my (undef,undef,undef,$sub)=caller(1);Carp::croak("attribute $sub is read-only")}sub __CHECK {if (my@missing=grep {not exists $_[0]->{$_}}'class'){Carp::croak('OptArgs2::Cmd attribute(s) required: ' .join(', ',@missing))}no strict 'refs';my$_attrs=*{ref($_[0]).'::_ATTRS'};map {delete$_attrs->{$_}}keys %{$OptArgs2::Cmd::_CI->{'has'}}}sub class {$_[0]->__RO($_[1])if @_ > 1;$_[0]{'class'}}sub name {$_[0]->__RO($_[1])if @_ > 1;$_[0]{'name'}//= $OptArgs2::Cmd::_CI->{'has'}->{'name'}->{'default'}->($_[0])}sub _dump {my$self=shift;my$d=shift // 1;require Data::Dumper;no warnings 'once';local$Data::Dumper::Indent=1;local$Data::Dumper::Maxdepth=$d;local$Data::Dumper::Sortkeys=1;my$x=Data::Dumper::Dumper($self);$x =~ s/.*?{/{/;$x =~ s/}.*?\n$/}/;my$i=0;my@list;do {@list=caller($i++)}until$list[3]eq 'OptArgs2::Cmd::_dump';warn "$self $x at $list[1]:$list[2]\n"}BEGIN {$INC{'OptArgs2/Cmd.pm'}=__FILE__}
CI_PM_FILTER
1;
